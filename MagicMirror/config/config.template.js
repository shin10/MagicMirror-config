	...
	modules: [
		{
			...
			config: {
				...
				onDetected: {
					...
					shellExec: "vcgencmd display_power 1; mosquitto_pub -h home.broker -t /magic-mirror/display_power -m 1",
						...
						fires: [
							{
								fire: "PAGE_ROTATION",
								exec: (payload) => {
									MM.sendNotification(payload ? "RESUME_ROTATION" : "PAUSE_ROTATION", payload, MM.getModules()[0]);
								}
							},
							{
...

		{
			...
			config: {
				...
				modules: [
					...
					["MMM-FF-Evan-Roth-Red-Lines"],
					["alert", "clock", "calendar-holidays", "weather-current", "weather-forecast", "MMM-NINA", "MMM-FF-Dilbert", "newsfeed-times"],
					["alert", "clock", "calendar-private", "weather-current", "weather-hourly", "MMM-NINA", "MMM-wiki"],
					["updatenotification", "MMM-text-clock"]
				],
				hidden: ["MMM-Touch"],
				fixed: ["MMM-PIR-Sensor", "MMM-GroveGestures", "MMM-page-indicator", "MMM-FF-StatsJS", "MMM-FF-process-stats", "MMM-SystemStats"]
			}
		}
	]
};
